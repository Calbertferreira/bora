...
import { formatEventLocation, resolveEventLocation } from "@/lib/event-locations";
...
const schema = z.object({ clientContactId: z.uuid().optional().or(z.literal("")), clientName: z.string().trim().min(2).max(160), clientEmail: z.email(), clientWhatsapp: z.string().trim().min(10).max(30).refine(isValidWhatsapp, "Informe um WhatsApp válido."), eventLocationId: z.uuid().optional().or(z.literal("")), locationName: z.string().trim().max(160).optional().default(""), locationAddress: z.string().trim().max(240).optional().default(""), locationCity: z.string().trim().max(100).optional().default(""), locationState: z.string().trim().max(2).optional().default(""), eventType: z.string().trim().min(2).max(100), eventTitle: z.string().trim().min(3).max(160), eventDate: z.iso.date().optional().or(z.literal("")), sendToClient: z.boolean().default(false), items: z.array(itemSchema).min(1).max(50) });
...
export async function POST(request: Request) {
  ...
  let location;
  try { location = await resolveEventLocation(context.db, data, ownerId); }
  catch { return Response.json({ error: "Selecione um local ou informe nome e endereço completos." }, { status: 400 }); }
  const [venue] = await context.db.select({ id: supplierListings.id }).from(supplierListings).where(and(eq(supplierListings.eventLocationId, location.id), eq(supplierListings.type, "VENUE"))).limit(1);
  const amounts = calculateEmbeddedCommission(normalizedItems.reduce((sum, item) => sum + item.amountCents, 0), context.supplier.administrationFeeBps); const status = data.sendToClient ? "SENT" : "DRAFT";
  const [created] = await context.db.insert(eventPackages).values({ organizerSupplierId: ownerId, clientUserId: client?.id ?? null, clientContactId: contact.id, clientName: contact.name, clientEmail: contact.email, clientWhatsapp: contact.whatsappNumber, eventType: data.eventType, eventTitle: data.eventTitle, eventDate: data.eventDate || null, eventLocation: formatEventLocation(location), eventLocationId: location.id, venueListingId: venue?.id ?? null, origin: "SUPPLIER", status, subtotalCents: amounts.subtotalCents, administrationFeeBps: context.supplier.administrationFeeBps, administrationFeeCents: amounts.commissionCents, totalCents: amounts.totalCents, sentAt: data.sendToClient ? new Date() : null }).returning({ id: eventPackages.id });
  ...
  await context.db.insert(auditLogs).values({ actorUserId: ownerId, targetUserId: client?.id ?? null, action: data.sendToClient ? "SUPPLIER_PACKAGE_SENT" : "SUPPLIER_PACKAGE_CREATED", details: { packageId: created.id, totalCents: amounts.totalCents, commissionCents: amounts.commissionCents, clientLinked: Boolean(client) } });
