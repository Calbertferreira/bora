...
function safeReturnPath(value?: string) { return value?.startsWith("/") && !value.startsWith("//") ? value : "/painel"; }

export default async function SignupPage({ searchParams }: { searchParams: Promise<{ perfil?: string; retorno?: string }> }) {
  const params = await searchParams;
  const callbackURL = safeReturnPath(params.retorno);
  ...
  return <main className="auth-page">
    ...
    <section className="auth-card wide">
      ...
      <SignupForm googleEnabled={isGoogleAuthEnabled} initialRole={initialRole} serviceSuggestions={serviceSuggestions} callbackURL={callbackURL} />
    </section>
  </main>;
}
