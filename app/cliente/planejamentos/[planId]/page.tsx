import { and, asc, eq, gte, ilike, isNull, or } from "drizzle-orm";
import { ArrowLeft, CalendarDays, Check, MapPin, Users } from "lucide-react";
import { notFound } from "next/navigation";
import { requirePageRole } from "@/lib/access";
import { getDb } from "@/lib/db";
import { plans, planServices, planStatusHistory, supplierListingImages, supplierListings, supplierProfiles } from "@/lib/db/schema";
...
export default async function ClientPlanDetailPage({ params }: { params: Promise<{ planId: string }> }) {
  ...
  return <main className="client-page"><section className="client-shell plan-detail-shell">
    ...
    <section className="plan-results"><div><span>RESULTADOS</span><h2>Opções compatíveis</h2><p>Ofertas publicadas que atendem ao local e à quantidade de pessoas informados.</p></div>{suggestions.length ? <div className="result-grid">{suggestions.map((item) => <article key={item.id}>{item.imageUrl ? <img src={item.imageUrl} alt={item.name} /> : <div className="result-placeholder">bora?</div>}<div><small>{item.businessName}</small><h3>{item.name}</h3><p>{item.description}</p><strong>{priceUnits[item.priceUnit] === "a partir de" ? "A partir de " : ""}{new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(item.priceCents / 100)} <small>{priceUnits[item.priceUnit] !== "a partir de" ? priceUnits[item.priceUnit] : ""}</small></strong></div></article>)}</div> : <div className="results-waiting"><h3>A equipe BORA está buscando as melhores opções</h3><p>Você verá aqui os locais, serviços e propostas compatíveis assim que estiverem disponíveis.</p></div>}</section>
  </section></main>;
}
