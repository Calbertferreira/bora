...
export function SignupForm({ googleEnabled, initialRole }: { googleEnabled: boolean; initialRole: PublicRole }) {
  ...
  async function signup(event: FormEvent<HTMLFormElement>) {
    ...
    if (!onboarding.ok) {
      ...
      window.setTimeout(() => { window.location.href = "/onboarding"; }, 1200);
      return;
    }
  ...
  return <form className="auth-form" onSubmit={signup}>
    ...
    <div className="form-grid">
      ...
      {role === "SUPPLIER" && <>
        <label>Nome comercial<input name="businessName" required minLength={2} /></label>
        <label>Categoria do serviço<input name="serviceCategory" placeholder="Ex.: Buffet, transporte" required minLength={2} /></label>
      </>}
      <label>Senha<input name="password" type="password" autoComplete="new-password" minLength={8} required /></label>
      <label>Confirmar senha<input name="passwordConfirmation" type="password" autoComplete="new-password" minLength={8} required /></label>
    </div>
    ...
    <button className="social-button" type="button" onClick={signupWithGoogle} disabled={!googleEnabled || loading}>
      <span className="google-mark">G</span>
      {googleEnabled ? "Cadastrar com Google" : "Google disponível em breve"}
    </button>
